...
export default function DoctorsPage() {
  ...
  function generateDoctorId(existing: Doctor[]) {
    ...
    const nums = existing
      .map((d) => Number.parseInt(d.id.replace(/\D/g, ""), 10))
      .filter((n) => !isNaN(n));
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (form.id) {
        ...
        setDoctors((prev) => {
          const next = prev.map((d) =>
            d.id === form.id ? ({ ...d, ...updated } as Doctor) : d
          );
      ...
      alert(
        "Failed to save doctor. Please check Supabase table and RLS policies."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      alert(
        "Failed to delete doctor. Please check Supabase table and RLS policies."
      );
    } finally {
      setDeletingId(null);
    }
